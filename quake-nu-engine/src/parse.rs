...
use nu_protocol::{span, Category, DeclId, Spanned, SyntaxShape, Type};

use quake_core::metadata::{Task, TaskFlags};
use quake_core::prelude::*;

use crate::commands::ResolvedDependency;
...
fn parse_def_task(
    ...
    for expr in call.arguments.iter_mut().flat_map(|a| a.expression_mut()) {
        ...
        let block_id = match &kw_expr.expr {
            // extract the block if successfully parsed
            Expr::Closure(block_id) => *block_id,
            // otherwise, try reparsing
            Expr::Garbage => {
    ...
    // error if no blocks are provided
    if decl_body.is_none() && run_body.is_none() {
        state.error(errors::TaskMissingBlocks { span: call_span });
        return Ok(());
    }

    // apply the signature to both blocks
    ...
    let depends_decl_id = {
        ...
        let decl_id = working_set.add_decl(Box::new(ResolvedDependency { task_id, signature }));
        working_set
            .last_overlay_mut()
            .visibility
    ...
    if let Some(decl_body) = decl_body {
        ...
        // transform `Dependency` calls to `ResolvedDency`
        modify_calls(
            working_set,
            b"dependency",
            &mut block,
    ...
    // try to register the task (errors if already defined)
